...
main :: IO ()
main = Options.parse >>= runReaderT
    (runExceptT program >>= either handleProgramError handleProgramResult)
...
writeResDerivation
    :: DerivAST.RenderedDerivation -> ReaderT Options.Options IO ()
...
parseAsSyntaxSelect = withExceptT ErrParseError . except . M.parse
    parser
    "<stdin>"
  where
...
typeDeriveExpr =
    withExceptT ErrTypeDerivError . except . TypeCheck.getTypeDerivation
...
renderTypeDerivation (_, deriv) = do
    ...
    case renderer of
        RLatex cfg ->
            let parsedExpr = RendererLatex.renderLatex cfg deriv
            in  return $ ResDerivation parsedExpr
