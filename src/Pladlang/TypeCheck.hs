...
typeCheck expr = case expr of
    EMeta e t -> do
        ...
        let sequent =
                formSequent bindings (exprToDerivExpr expr) (typeToDerivType t')
            rule = DerivAST.SequentOnly $ sequent
        return (t, rule)
    ETrue -> do
        bindings <- asks envBindings
        let sequent = formSequent bindings
                                  (exprToDerivExpr ETrue)
                                  (typeToDerivType TBool)
            rule = validRule "bool" [] sequent
        return (Just TBool, rule)
    EFalse -> do
        bindings <- asks envBindings
        let sequent = formSequent bindings
                                  (exprToDerivExpr EFalse)
                                  (typeToDerivType TBool)
            rule = validRule "bool" [] sequent
        return (Just TBool, rule)
    EVar v -> do
        ...
        case Map.lookup v bindings of
            ...
            Nothing ->
                let
                    sequent = formSequent bindings
                                          (exprToDerivExpr expr)
                                          derivASTEmptyMetaType
                    rule = invalidRule
                        (DerivAST.TypeErrorUndefinedVariableUsed v)
                        sequent
                in
                    return (Nothing, rule)
    ENum _ -> do
    ...
    ECat   e1 e2 -> do
        ...
        case e1Typ of
            ...
            _ ->
                let
                    e2Sequent = formSequent bindings
                                            (exprToDerivExpr e2)
                                            (typeToDerivType TStr)
    ...
    ELen e -> do
        ...
        case eTyp of
            ...
            Just t ->
                let
                    rule = invalidRule
                        (DerivAST.TypeErrorArgWrongType (typeToDerivType t))
                        sequent
                in  return (Nothing, rule)
    ELet e1 v e2 -> do
        (e1Typ, e1Rule) <- typeCheck e1
        case e1Typ of
            Nothing -> do
                ...
                --rule = invalidRule (DerivAST.TypeErrorArgWrongType derivASTEmptyMetaType) sequent
                    e2SequentContext =
                        ( DerivAST.Binding v derivASTEmptyMetaType
                        : bindingsToDerivContext bindings
                        )
                    e2Sequent = DerivAST.Sequent
                        { DerivAST.sequentContext = e2SequentContext
                        , DerivAST.sequentExpr    = exprToDerivExpr e2
                        , DerivAST.sequentType    = DerivAST.TMeta "1"
                        }
                    e2Rule = DerivAST.SequentOnly $ e2Sequent
                    rule   = validRule "let" [e1Rule, e2Rule] sequent
                return (Nothing, rule)
            Just e1Typ' -> do
                (e2Typ, e2Rule) <- local (addTypeBinding v e1Typ')
                                         (typeCheck e2)
                case e2Typ of
                    Nothing -> do
                        bindings <- asks envBindings
    ...
    ELam t v e -> do
        ...
        case eTyp of
            Nothing ->
                let sequent =
                            untypedSequent $ derivTypePart derivASTEmptyMetaType
                    rule = validRule "lam" [eRule] sequent
                in  return (Nothing, rule)
            Just eTyp' ->
                let
                    sequent =
                        untypedSequent $ derivTypePart (typeToDerivType eTyp')
                    rule = validRule "lam" [eRule] sequent
                in
                    return (Just (TArrow t eTyp'), rule)
    EAp e1 e2 -> do
        bindings <- asks envBindings
        ...
        case e1Typ of
            Nothing ->
                let sequent = untypedSequent $ derivASTEmptyMetaType
                    e2Rule  = DerivAST.SequentOnly $ formSequent
                        bindings
                        (exprToDerivExpr e2)
                        (DerivAST.TMeta "1")
            ...
            Just (TArrow tArg tBody) -> do
                ...
                let sequent = untypedSequent $ typeToDerivType tBody
                    rule    = validRule "ap" [e1Rule, e2Rule] sequent
                case e2Typ of
                    Nothing     -> return (Nothing, rule)
                    Just e2Typ' -> if e2Typ' == tArg
                        then return (Just tBody, rule)
...
exprToDerivExpr = \case
    ...
    ELen e       -> DerivAST.EFunc "len" [exprToDerivExpr e]
    ELet e1 v e2 -> DerivAST.ELet (exprToDerivExpr e1) v (exprToDerivExpr e2)
    ELam t  v e  -> DerivAST.ELam (typeToDerivType t) v (exprToDerivExpr e)
    EAp e1 e2    -> DerivAST.EFunc "ap" [exprToDerivExpr e1, exprToDerivExpr e2]
    _            -> DerivAST.EMeta ""

validRule :: Text -> [DerivAST.Rule] -> DerivAST.Sequent -> DerivAST.Rule
validRule name premises judgement = DerivAST.ValidRule DerivAST.ValidRule'
...
invalidRule err judgement = DerivAST.InvalidRule DerivAST.InvalidRule'
...
formSequent bindings expr typ =
    let context = bindingsToDerivContext bindings
    in  DerivAST.Sequent { DerivAST.sequentContext = context
                         , DerivAST.sequentExpr    = expr
                         , DerivAST.sequentType    = typ
                         }

bindingsToDerivContext :: Map Text Type -> [DerivAST.ContextPart]
bindingsToDerivContext bs = if Map.null bs
    then []
    else map (\(v, t) -> DerivAST.Binding v (typeToDerivType t)) (Map.assocs bs)
